<!-- Source: https://docs.openclaw.ai/zh-CN/concepts/messages -->
<!-- Title: 消息 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/concepts/messages)
...
-   [消息流程（高层概述）](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [入站去重](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [入站防抖](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [会话和设备](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [入站正文和历史上下文](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [队列和后续消息](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [流式传输、分块和批处理](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [推理可见性和 token](https://docs.openclaw.ai/zh-CN/concepts/messages)
-   [前缀、线程和回复](https://docs.openclaw.ai/zh-CN/concepts/messages)
...
完整 schema 参见[配置](https://docs.openclaw.ai/gateway/configuration)。
...
来自**同一发送者**的快速连续消息可以通过 `messages.inbound` 批量合并为单个智能体轮次。防抖按渠道 + 会话为范围，并使用最近的消息进行回复线程/ID 处理。 配置（全局默认 + 单渠道覆盖）：
...
多个设备/渠道可以映射到同一会话，但历史记录不会完全同步回每个客户端。建议：对长对话使用一个主设备，以避免上下文分歧。控制 UI 和 TUI 始终显示 Gateway 网关支持的会话记录，因此它们是事实来源。 详情：[会话管理](https://docs.openclaw.ai/concepts/session)。
...
对于**非直接聊天**（群组/渠道/房间），**当前消息正文**会加上发送者标签前缀（与历史条目使用的样式相同）。这使智能体提示中的实时消息和队列/历史消息保持一致。 历史缓冲区是**仅待处理的**：它们包含_未_触发运行的群组消息（例如，提及门控的消息），并**排除**已在会话记录中的消息。 指令剥离仅适用于**当前消息**部分，因此历史记录保持完整。包装历史记录的渠道应将 `CommandBody`（或 `RawBody`）设置为原始消息文本，并将 `Body` 保留为组合提示。历史缓冲区可通过 `messages.groupChat.historyLimit`（全局默认）和单渠道覆盖（如 `channels.slack.historyLimit` 或 `channels.telegram.accounts.<id>.historyLimit`）进行配置（设置 `0` 表示禁用）。
...
详情：[队列](https://docs.openclaw.ai/concepts/queue)。
...
分块流式传输在模型生成文本块时发送部分回复。分块遵循渠道文本限制，避免拆分围栏代码。 关键设置：
...
详情：[流式传输 + 分块](https://docs.openclaw.ai/concepts/streaming)。
...
详情：[思考 + 推理指令](https://docs.openclaw.ai/tools/thinking)和 [Token 使用](https://docs.openclaw.ai/reference/token-use)。
...
详情：[配置](https://docs.openclaw.ai/gateway/configuration)和渠道文档。

[在线状态](https://docs.openclaw.ai/zh-CN/concepts/presence)[流式传输和分块](https://docs.openclaw.ai/zh-CN/concepts/streaming)
