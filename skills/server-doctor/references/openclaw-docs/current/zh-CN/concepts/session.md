<!-- Source: https://docs.openclaw.ai/zh-CN/concepts/session -->
<!-- Title: 会话管理 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/concepts/session)
...
-   [Gateway 网关是唯一数据源](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [状态存储位置](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [会话修剪](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [压缩前记忆刷新](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [传输到会话键的映射](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [生命周期](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [发送策略（可选）](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [配置（可选重命名示例）](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [检查](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [提示](https://docs.openclaw.ai/zh-CN/concepts/session)
-   [会话来源元数据](https://docs.openclaw.ai/zh-CN/concepts/session)
...
OpenClaw 将**每个智能体的一个直接聊天会话**视为主会话。直接聊天折叠为 `agent:<agentId>:<mainKey>`（默认 `main`），而群组/频道聊天获得各自的键。`session.mainKey` 会被遵循。 使用 `session.dmScope` 控制**私信**如何分组：

-   `main`（默认）：所有私信共享主会话以保持连续性。
-   `per-peer`：跨渠道按发送者 ID 隔离。
-   `per-channel-peer`：按渠道 + 发送者隔离（推荐用于多用户收件箱）。
-   `per-account-channel-peer`：按账户 + 渠道 + 发送者隔离（推荐用于多账户收件箱）。 使用 `session.identityLinks` 将带提供商前缀的对等 ID 映射到规范身份，这样在使用 `per-peer`、`per-channel-peer` 或 `per-account-channel-peer` 时，同一个人可以跨渠道共享私信会话。
...
默认情况下，OpenClaw 在 LLM 调用之前从内存上下文中修剪**旧的工具结果**。 这**不会**重写 JSONL 历史记录。参见 [/concepts/session-pruning](https://docs.openclaw.ai/concepts/session-pruning)。
...
当会话接近自动压缩时，OpenClaw 可以运行一个**静默记忆刷新**轮次，提醒模型将持久性笔记写入磁盘。这仅在工作区可写时运行。参见[记忆](https://docs.openclaw.ai/concepts/memory)和[压缩](https://docs.openclaw.ai/concepts/compaction)。
...
-   `/send on` → 为此会话允许
-   `/send off` → 为此会话拒绝
-   `/send inherit` → 清除覆盖并使用配置规则 将这些作为独立消息发送以使其生效。
...
-   单独发送 `/compact`（可选指令）消息可总结旧上下文并释放窗口空间。参见 [/concepts/compaction](https://docs.openclaw.ai/concepts/compaction)。
...
-   `threadId`：渠道支持时的线程/话题 ID 来源字段为私信、频道和群组填充。如果连接器仅更新投递路由（例如，保持私信主会话新鲜），它仍应提供入站上下文，以便会话保留其解释器元数据。扩展可以通过在入站上下文中发送 `ConversationLabel`、`GroupSubject`、`GroupChannel`、`GroupSpace` 和 `SenderName` 并调用 `recordSessionMetaFromInbound`（或将相同上下文传递给 `updateLastRoute`）来实现。

[引导](https://docs.openclaw.ai/zh-CN/start/bootstrapping)[Session pruning](https://docs.openclaw.ai/zh-CN/concepts/session-pruning)
