<!-- Source: https://docs.openclaw.ai/zh-CN/concepts/queue -->
<!-- Title: 命令队列 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/concepts/queue)
...
-   [命令队列（2026-01-16）](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [为什么需要](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [工作原理](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [队列模式（按渠道）](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [队列选项](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [按会话覆盖](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [范围和保证](https://docs.openclaw.ai/zh-CN/concepts/queue)
-   [故障排除](https://docs.openclaw.ai/zh-CN/concepts/queue)
...
steer-backlog 意味着你可以在被引导的运行之后获得后续响应，因此流式传输界面可能看起来像重复。如果你希望每条入站消息只有一个响应，请优先使用 `collect`/`steer`。 发送 `/queue collect` 作为独立命令（按会话）或设置 `messages.queue.byChannel.discord: "collect"`。 默认值（配置中未设置时）：
...
summarize 保留被丢弃消息的简短要点列表，并将其作为合成的后续提示注入。 默认值：`debounceMs: 1000`、`cap: 20`、`drop: summarize`。
