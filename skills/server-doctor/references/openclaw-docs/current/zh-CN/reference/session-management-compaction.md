<!-- Source: https://docs.openclaw.ai/zh-CN/reference/session-management-compaction -->
<!-- Title: 会话管理深入了解 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
...
-   [会话管理与压缩（深入了解）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [事实来源：Gateway 网关](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [两个持久化层](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [磁盘上的位置](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [会话键（sessionKey）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [会话 ID（sessionId）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [会话存储模式（sessions.json）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [记录结构（\*.jsonl）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [上下文窗口 vs 跟踪的 token](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [压缩：它是什么](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [自动压缩何时发生（Pi 运行时）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [压缩设置（reserveTokens、keepRecentTokens）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [用户可见的界面](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [静默内务处理（NO\_REPLY）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [压缩前”记忆刷新”（已实现）](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
-   [故障排除检查清单](https://docs.openclaw.ai/zh-CN/reference/session-management-compaction)
...
`sessionKey` 标识你所在的_哪个对话桶_（路由 + 隔离）。 常见模式：
...
规范规则记录在 [/concepts/session](https://docs.openclaw.ai/concepts/session)。
...
每个 `sessionKey` 指向一个当前的 `sessionId`（继续对话的记录文件）。 经验法则：
...
存储的值类型是 `src/config/sessions.ts` 中的 `SessionEntry`。 关键字段（不完整）：
...
记录由 `@mariozechner/pi-coding-agent` 的 `SessionManager` 管理。 文件是 JSONL 格式：
...
更多信息，参见 [/token-use](https://docs.openclaw.ai/reference/token-use)。
...
压缩将较旧的对话总结为记录中的持久化 `compaction` 条目，并保持最近的消息不变。 压缩后，未来的回合会看到：

-   压缩摘要
-   `firstKeptEntryId` 之后的消息

压缩是**持久化的**（与会话修剪不同）。参见 [/concepts/session-pruning](https://docs.openclaw.ai/concepts/session-pruning)。
...
原因：为压缩变得不可避免之前的多回合”内务处理”（如记忆写入）留出足够的空间。 实现：`src/agents/pi-settings.ts` 中的 `ensurePiCompactionReserveTokens()`（从 `src/agents/pi-embedded-runner.ts` 调用）。
...
OpenClaw 支持用于后台任务的”静默”回合，用户不应该看到中间输出。 约定：
...
目标：在自动压缩发生之前，运行一个静默的智能体回合，将持久状态写入磁盘（例如智能体工作空间中的 `memory/YYYY-MM-DD.md`），这样压缩就不会擦除关键上下文。 OpenClaw 使用**预阈值刷新**方法：
...
-   参见[记忆](https://docs.openclaw.ai/concepts/memory)了解工作空间文件布局和写入模式。
...
-   会话键错误？从 [/concepts/session](https://docs.openclaw.ai/concepts/session) 开始，并在 `/status` 中确认 `sessionKey`。
