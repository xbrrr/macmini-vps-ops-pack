<!-- Source: https://docs.openclaw.ai/zh-CN/tools/browser -->
<!-- Title: 浏览器（OpenClaw 托管） - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/tools/browser)
...
-   [功能概览](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [快速开始](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [配置文件：openclaw 与 chrome](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [配置](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [使用 Brave（或其他基于 Chromium 的浏览器）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [本地控制与远程控制](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [节点浏览器代理（零配置默认）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [Browserless（托管远程 CDP）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [安全性](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [配置文件（多浏览器）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [Chrome 扩展中继（使用你现有的 Chrome）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [沙箱会话](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [设置](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [隔离保证](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [浏览器选择](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [控制 API（可选）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [Playwright 要求](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [Docker Playwright 安装](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [工作原理（内部）](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [CLI 快速参考](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [快照和 ref](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [等待增强功能](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [调试工作流](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [JSON 输出](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [状态和环境开关](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [安全与隐私](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [故障排除](https://docs.openclaw.ai/zh-CN/tools/browser)
-   [智能体工具 + 控制工作原理](https://docs.openclaw.ai/zh-CN/tools/browser)
...
OpenClaw 可以运行一个由智能体控制的**专用 Chrome/Brave/Edge/Chromium 配置文件**。 它与你的个人浏览器隔离，通过 Gateway 网关内部的小型本地控制服务进行管理（仅限 loopback）。 新手视角：
...
如果你的**系统默认**浏览器是基于 Chromium 的（Chrome/Brave/Edge 等），OpenClaw 会自动使用它。设置 `browser.executablePath` 可覆盖自动检测： CLI 示例：
...
如果你在有浏览器的机器上运行**节点主机**，OpenClaw 可以自动将浏览器工具调用路由到该节点，无需任何额外的浏览器配置。这是远程 Gateway 网关的默认路径。 注意事项：
...
[Browserless](https://browserless.io/) 是一个托管的 Chromium 服务，通过 HTTPS 暴露 CDP 端点。你可以将 OpenClaw 浏览器配置文件指向 Browserless 区域端点，并使用你的 API 密钥进行认证。 示例：
...
OpenClaw 还可以通过本地 CDP 中继 + Chrome 扩展驱动**你现有的 Chrome 标签页**（无需单独的”openclaw”Chrome 实例）。 完整指南：[Chrome 扩展](https://docs.openclaw.ai/tools/chrome-extension) 流程：
...
你可以使用 `browser.executablePath` 覆盖。 平台：
...
某些功能（navigate/act/AI 快照/角色快照、元素截图、PDF）需要 Playwright。如果未安装 Playwright，这些端点会返回明确的 501 错误。ARIA 快照和基本截图对于 openclaw 托管的 Chrome 仍然有效。对于 Chrome 扩展中继驱动程序，ARIA 快照和截图需要 Playwright。 如果你看到 `Playwright is not available in this gateway build`，请安装完整的 Playwright 包（不是 `playwright-core`）并重启 Gateway 网关，或者重新安装带浏览器支持的 OpenClaw。
...
要持久化浏览器下载，设置 `PLAYWRIGHT_BROWSERS_PATH`（例如 `/home/node/.cache/ms-playwright`）并确保 `/home/node` 通过 `OPENCLAW_HOME_VOLUME` 或绑定挂载持久化。参见 [Docker](https://docs.openclaw.ai/install/docker)。
...
所有命令接受 `--browser-profile <name>` 以定位特定配置文件。 所有命令也接受 `--json` 以获得机器可读的输出（稳定的负载）。 基础操作：
...
-   `snapshot`：
    ...
    -   配置默认值（仅限工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"` 以在调用者未传递模式时使用高效快照（参见 [Gateway 网关配置](https://docs.openclaw.ai/gateway/configuration)）。
...
-   有关登录和反机器人注意事项（X/Twitter 等），请参阅 [浏览器登录 + X/Twitter 发帖](https://docs.openclaw.ai/tools/browser-login)。
...
有关 Linux 特定问题（特别是 snap Chromium），请参阅[浏览器故障排除](https://docs.openclaw.ai/tools/browser-linux-troubleshooting)。
...
[Web 工具](https://docs.openclaw.ai/zh-CN/tools/web)[浏览器登录](https://docs.openclaw.ai/zh-CN/tools/browser-login)
