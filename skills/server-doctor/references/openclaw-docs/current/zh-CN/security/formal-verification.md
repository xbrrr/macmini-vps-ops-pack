<!-- Source: https://docs.openclaw.ai/zh-CN/security/formal-verification -->
<!-- Title: 形式化验证（安全模型） - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/security/formal-verification)
...
-   [形式化验证（安全模型）](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [模型存放位置](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [重要注意事项](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [复现结果](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [Gateway 网关暴露和开放 Gateway 网关配置错误](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [Nodes.run 管道（最高风险能力）](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [配对存储（私信门控）](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [入站门控（提及 + 控制命令绕过）](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [路由/会话键隔离](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [v1++：额外的有界模型（并发、重试、追踪正确性）](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [配对存储并发 / 幂等性](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [入站追踪关联 / 幂等性](https://docs.openclaw.ai/zh-CN/security/formal-verification)
-   [路由 dmScope 优先级 + identityLinks](https://docs.openclaw.ai/zh-CN/security/formal-verification)
...
**目标（北极星）：** 提供机器检查的论证，证明 OpenClaw 在明确假设下执行其 预期的安全策略（授权、会话隔离、工具门控和 配置错误安全）。 **目前是什么：** 一个可执行的、攻击者驱动的**安全回归测试套件**：
...
**声明：** 配对存储应该在交错情况下也强制执行 `MaxPending` 和幂等性（即”检查然后写入”必须是原子/加锁的；刷新不应创建重复项）。 这意味着：
...
**声明：** 摄入应在扇出过程中保持追踪关联，并在提供商重试下保持幂等。 这意味着：
...
**声明：** 路由必须默认保持私信会话隔离，只有在明确配置时才折叠会话（渠道优先级 + 身份链接）。 这意味着：
...
[日志](https://docs.openclaw.ai/zh-CN/logging)[Web](https://docs.openclaw.ai/zh-CN/web)
