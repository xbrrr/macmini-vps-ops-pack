<!-- Source: https://docs.openclaw.ai/zh-CN/gateway/doctor -->
<!-- Title: Doctor - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/gateway/doctor)
...
-   [Doctor](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [快速开始](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [无头/自动化](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [功能概述](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [详细行为和原理](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [0）可选更新（git 安装）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [1）配置规范化](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [2）遗留配置键迁移](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [2b）OpenCode Zen 提供商覆盖](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [3）遗留状态迁移（磁盘布局）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [4）状态完整性检查（会话持久化、路由和安全）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [5）模型认证健康（OAuth 过期）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [6）Hooks 模型验证](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [7）沙箱镜像修复](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [8）Gateway 网关服务迁移和清理提示](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [9）安全警告](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [10）systemd linger（Linux）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [11）Skills 状态](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [12）Gateway 网关认证检查（本地 token）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [13）Gateway 网关健康检查 + 重启](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [14）渠道状态警告](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [15）Supervisor 配置审计 + 修复](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [16）Gateway 网关运行时 + 端口诊断](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [17）Gateway 网关运行时最佳实践](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [18）配置写入 + 向导元数据](https://docs.openclaw.ai/zh-CN/gateway/doctor)
-   [19）工作区提示（备份 + 记忆系统）](https://docs.openclaw.ai/zh-CN/gateway/doctor)
...
扫描系统服务以查找额外的 Gateway 网关安装（launchd/systemd/schtasks）。 如果你想在写入前查看更改，请先打开配置文件：
...
当配置包含已弃用的键时，其他命令会拒绝运行并要求你运行 `openclaw doctor`。 Doctor 将：

-   解释找到了哪些遗留键。
-   显示它应用的迁移。
-   使用更新后的 schema 重写 `~/.openclaw/openclaw.json`。

Gateway 网关在检测到遗留配置格式时也会在启动时自动运行 doctor 迁移，因此过时的配置无需手动干预即可修复。 当前迁移：
...
状态目录是操作的核心。如果它消失，你会丢失会话、凭证、日志和配置（除非你在别处有备份）。 Doctor 检查：
...
Doctor 检查认证存储中的 OAuth 配置文件，在 token 即将过期/已过期时发出警告，并在安全时刷新它们。如果 Anthropic Claude Code 配置文件过时，它会建议运行 `claude setup-token`（或粘贴 setup-token）。 刷新提示仅在交互运行（TTY）时出现；`--non-interactive` 跳过刷新尝试。 Doctor 还会报告由于以下原因暂时不可用的认证配置文件：
...
Doctor 检测遗留的 Gateway 网关服务（launchd/systemd/schtasks），并提供删除它们并使用当前 Gateway 网关端口安装 OpenClaw 服务的选项。它还可以扫描额外的类 Gateway 网关服务并打印清理提示。 配置文件命名的 OpenClaw Gateway 网关服务被视为一等公民，不会被标记为”额外的”。
...
Doctor 检查已安装的 supervisor 配置（launchd/systemd/schtasks）是否有缺失或过时的默认值（例如 systemd network-online 依赖和重启延迟）。当发现不匹配时，它会推荐更新，并可以将服务文件/任务重写为当前默认值。 注意事项：
...
当缺失时，Doctor 建议使用工作区记忆系统，并在工作区尚未在 git 下时打印备份提示。 参见 [/concepts/agent-workspace](https://docs.openclaw.ai/concepts/agent-workspace) 了解工作区结构和 git 备份的完整指南（推荐私有 GitHub 或 GitLab）。

[心跳](https://docs.openclaw.ai/zh-CN/gateway/heartbeat)[日志](https://docs.openclaw.ai/zh-CN/gateway/logging)
