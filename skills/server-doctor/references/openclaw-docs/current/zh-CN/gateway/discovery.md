<!-- Source: https://docs.openclaw.ai/zh-CN/gateway/discovery -->
<!-- Title: 设备发现 + 传输协议 - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/gateway/discovery)
...
-   [设备发现 & 传输协议](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [术语](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [为什么我们同时保留”直连”和 SSH](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [发现输入（客户端如何了解 Gateway 网关位置）](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [1）Bonjour / mDNS（仅限 LAN）](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [服务信标详情](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [2）Tailnet（跨网络）](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [3）手动 / SSH 目标](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [传输选择（客户端策略）](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [配对 + 认证（直连传输）](https://docs.openclaw.ai/zh-CN/gateway/discovery)
-   [各组件职责](https://docs.openclaw.ai/zh-CN/gateway/discovery)
...
-   **旧版 TCP 桥接（已弃用/移除）**：旧的节点传输（参见 [桥接协议](https://docs.openclaw.ai/gateway/bridge-protocol)）；不再用于发现广播。

协议详情：

-   [Gateway 网关协议](https://docs.openclaw.ai/gateway/protocol)
-   [桥接协议（旧版）](https://docs.openclaw.ai/gateway/bridge-protocol)
...
Bonjour 是尽力而为的，不会跨网络。它仅用于”同一 LAN”的便利性。 目标方向：

-   **Gateway 网关**通过 Bonjour 广播其 WS 端点。
-   客户端浏览并显示”选择一个 Gateway 网关”列表，然后存储选定的端点。

故障排除和信标详情：[Bonjour](https://docs.openclaw.ai/gateway/bonjour)。
...
当没有直连路由（或直连被禁用）时，客户端始终可以通过 SSH 转发本地回环 Gateway 网关端口来连接。 参见 [远程访问](https://docs.openclaw.ai/gateway/remote)。
...
-   配对请求在 Gateway 网关中创建/批准/拒绝（参见 [Gateway 网关配对](https://docs.openclaw.ai/gateway/pairing)）。
-   Gateway 网关强制执行：
    -   认证（令牌 / 密钥对）
...
[Gateway 网关拥有的配对](https://docs.openclaw.ai/zh-CN/gateway/pairing)[Bonjour 设备发现](https://docs.openclaw.ai/zh-CN/gateway/bonjour)
