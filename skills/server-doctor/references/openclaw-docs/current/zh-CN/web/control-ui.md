<!-- Source: https://docs.openclaw.ai/zh-CN/web/control-ui -->
<!-- Title: 控制 UI - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/web/control-ui)
...
-   [控制 UI（浏览器）](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [快速打开（本地）](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [设备配对（首次连接）](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [目前可以做什么](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [聊天行为](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [Tailnet 访问（推荐）](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [集成 Tailscale Serve（首选）](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [绑定到 tailnet + token](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [不安全的 HTTP](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [构建 UI](https://docs.openclaw.ai/zh-CN/web/control-ui)
-   [调试/测试：开发服务器 + 远程 Gateway 网关](https://docs.openclaw.ai/zh-CN/web/control-ui)
...
如果页面加载失败，请先启动 Gateway 网关：`openclaw gateway`。 认证在 WebSocket 握手期间通过以下方式提供：

-   `connect.params.auth.token`
-   `connect.params.auth.password` 仪表板设置面板允许你存储 token；密码不会被持久化。 新手引导向导默认生成一个 Gateway 网关 token，所以在首次连接时将其粘贴到这里。
...
当你从新浏览器或设备连接到控制 UI 时，Gateway 网关需要**一次性配对批准** — 即使你在同一个 Tailnet 上且 `gateway.auth.allowTailscale: true`。这是防止未授权访问的安全措施。 **你会看到：** “disconnected (1008): pairing required” **批准设备：**
...
一旦批准，设备会被记住，除非你使用 `openclaw devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](https://docs.openclaw.ai/cli/devices) 了解 token 轮换和撤销。 **注意：**
...
如果你通过普通 HTTP 打开仪表板（`http://<lan-ip>` 或 `http://<tailscale-ip>`），浏览器在**非安全上下文**中运行并阻止 WebCrypto。默认情况下，OpenClaw **阻止**没有设备身份的控制 UI 连接。 **推荐修复：** 使用 HTTPS（Tailscale Serve）或在本地打开 UI：
...
这会为控制 UI 禁用设备身份 + 配对（即使在 HTTPS 上）。仅在你信任网络时使用。 参见 [Tailscale](https://docs.openclaw.ai/gateway/tailscale) 了解 HTTPS 设置指南。
...
远程访问设置详情：[远程访问](https://docs.openclaw.ai/gateway/remote)。
