<!-- Source: https://docs.openclaw.ai/zh-CN/channels/discord -->
<!-- Title: Discord - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/channels/discord)
...
-   [Discord（Bot API）](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [快速设置（新手）](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [目标](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [工作原理](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [配置写入](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [如何创建自己的机器人](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [1）创建 Discord 应用 + 机器人用户](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [2）启用 OpenClaw 需要的网关意图](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [3）生成邀请 URL（OAuth2 URL Generator）](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [4）获取 ID（服务器/用户/频道）](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [5）配置 OpenClaw](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [令牌](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [允许列表 + 频道路由](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [6）验证是否工作](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [故障排除](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [功能和限制](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [重试策略](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [配置](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [PluralKit（PK）支持](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [工具操作默认值](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [回复标签](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [工具操作](https://docs.openclaw.ai/zh-CN/channels/discord)
-   [安全与运维](https://docs.openclaw.ai/zh-CN/channels/discord)
...
12.  表情反应：智能体可以通过 `discord` 工具触发表情反应（受 `channels.discord.actions.*` 控制）。
     -   表情反应移除语义：参见 [/tools/reactions](https://docs.openclaw.ai/tools/reactions)。
...
默认情况下，允许 Discord 写入由 `/config set|unset` 触发的配置更新（需要 `commands.config: true`）。 禁用方式：
...
Discord 会阻止”特权意图”，除非你明确启用它们。 在 **Bot** → **Privileged Gateway Intents** 中启用：
...
除非你在调试并完全信任机器人，否则避免使用 **Administrator**。 复制生成的 URL，打开它，选择你的服务器，然后安装机器人。
...
多账户支持：使用 `channels.discord.accounts`，每个账户有自己的令牌和可选的 `name`。参见 [`gateway/configuration`](https://docs.openclaw.ai/gateway/configuration) 了解通用模式。
...
-   **Discord 中的执行审批**：Discord 支持私信中执行审批的**按钮 UI**（允许一次 / 始终允许 / 拒绝）。`/approve <id> ...` 仅用于转发的审批，不会解析 Discord 的按钮提示。如果你看到 `❌ Failed to submit approval: Error: unknown approval id` 或 UI 从未出现，请检查：
    ...
    -   参见[执行审批](https://docs.openclaw.ai/tools/exec-approvals)和[斜杠命令](https://docs.openclaw.ai/tools/slash-commands)了解更广泛的审批和命令流程。
...
出站 Discord API 调用在速率限制（429）时使用 Discord `retry_after`（如果可用）进行重试，采用指数退避和抖动。通过 `channels.discord.retry` 配置。参见[重试策略](https://docs.openclaw.ai/concepts/retry)。
...
[BlueBubbles](https://docs.openclaw.ai/zh-CN/channels/bluebubbles)[飞书](https://docs.openclaw.ai/zh-CN/channels/feishu)
