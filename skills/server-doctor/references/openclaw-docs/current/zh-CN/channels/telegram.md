<!-- Source: https://docs.openclaw.ai/zh-CN/channels/telegram -->
<!-- Title: Telegram - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/channels/telegram)
...
-   [Telegram（Bot API）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [快速设置（入门）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [这是什么](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [设置（快速路径）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [1）创建机器人 token（BotFather）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [2）配置 token（环境变量或配置文件）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [Token + 隐私 + 权限（Telegram 端）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [Token 创建（BotFather）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [群组消息可见性（隐私模式）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [群组权限（管理员权限）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [工作原理（行为）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [草稿流式传输](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [格式化（Telegram HTML）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [命令（原生 + 自定义）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [故障排除](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [限制](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [群组激活模式](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [通过配置（推荐）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [通过命令（会话级别）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [获取群组聊天 ID](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [配置写入](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [话题（论坛超级群组）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [内联按钮](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [发送按钮](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [配置选项](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [访问控制（私信 + 群组）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [私信访问](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [查找你的 Telegram 用户 ID](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [群组访问](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [长轮询 vs webhook](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [回复线程](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [音频消息（语音 vs 文件）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [贴纸](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [接收贴纸](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [贴纸缓存](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [发送贴纸](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [流式传输（草稿）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [重试策略](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [智能体工具（消息 + 反应）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [反应通知](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [投递目标（CLI/cron）](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [故障排除](https://docs.openclaw.ai/zh-CN/channels/telegram)
-   [配置参考（Telegram）](https://docs.openclaw.ai/zh-CN/channels/telegram)
...
环境变量选项：`TELEGRAM_BOT_TOKEN=...`（适用于默认账户）。 如果环境变量和配置都设置了，配置优先。 多账户支持：使用 `channels.telegram.accounts`，每个账户有独立的 token 和可选的 `name`。参见 [`gateway/configuration`](https://docs.openclaw.ai/gateway/configuration) 了解共享模式。
...
Telegram 机器人默认启用**隐私模式**，这会限制它们接收哪些群组消息。 如果你的机器人必须看到_所有_群组消息，有两个选项：
...
OpenClaw 可以在 Telegram 私信中使用 `sendMessageDraft` 流式传输部分回复。 要求：
...
更多帮助：[渠道故障排除](https://docs.openclaw.ai/channels/troubleshooting)。 注意：
...
**重要：** 设置 `channels.telegram.groups` 会创建一个**允许列表** - 只有列出的群组（或 `"*"`）会被接受。 论坛话题继承其父群组配置（allowFrom、requireMention、skills、prompts），除非你在 `channels.telegram.groups.<groupId>.topics.<topicId>` 下添加每话题覆盖。 要允许所有群组并始终响应：
...
将群组中的任何消息转发给 Telegram 上的 `@userinfobot` 或 `@getidsbot` 以查看聊天 ID（负数，如 `-1001234567890`）。 **提示：** 要获取你自己的用户 ID，私信机器人，它会回复你的用户 ID（配对消息），或者在命令启用后使用 `/whoami`。 **隐私注意：** `@userinfobot` 是第三方机器人。如果你更倾向于其他方式，将机器人添加到群组，发送一条消息，然后使用 `openclaw logs --follow` 读取 `chat.id`，或使用 Bot API `getUpdates`。
...
默认情况下，Telegram 允许写入由渠道事件或 `/config set|unset` 触发的配置更新。 这发生在以下情况：
...
-   配对是 Telegram 私信使用的默认 token 交换。详情：[配对](https://docs.openclaw.ai/channels/pairing)
...
两个独立的控制： **1\. 允许哪些群组**（通过 `channels.telegram.groups` 的群组允许列表）：
...
-   `"disabled"` = 不接受任何群组消息 默认是 `groupPolicy: "allowlist"`（除非添加 `groupAllowFrom` 否则被阻止）。
...
Telegram 区分**语音备忘录**（圆形气泡）和**音频文件**（元数据卡片）。 OpenClaw 默认使用音频文件以保持向后兼容性。 要在智能体回复中强制使用语音备忘录气泡，在回复中的任何位置包含此标签：

-   `[[audio_as_voice]]` — 将音频作为语音备忘录而不是文件发送。

该标签会从发送的文本中去除。其他渠道会忽略此标签。 对于消息工具发送，设置 `asVoice: true` 并配合兼容语音的音频 `media` URL（当存在 media 时 `message` 是可选的）：
...
**搜索贴纸：** 智能体可以按描述、表情符号或集合名称搜索缓存的贴纸：
...
搜索在描述文本、表情符号字符和集合名称之间使用模糊匹配。 **带线程的示例：**
...
Telegram 可以在智能体生成响应时流式传输**草稿气泡**。 OpenClaw 使用 Bot API `sendMessageDraft`（不是真实消息），然后将最终回复作为普通消息发送。 要求（Telegram Bot API 9.3+）：
...
注意：草稿流式传输与**分块流式传输**（渠道消息）不同。 分块流式传输默认关闭，如果你想要早期 Telegram 消息而不是草稿更新，需要 `channels.telegram.blockStreaming: true`。 推理流（仅限 Telegram）：

-   `/reasoning stream` 在回复生成时将推理流式传输到草稿气泡中，然后发送不带推理的最终答案。
-   如果 `channels.telegram.streamMode` 为 `off`，推理流被禁用。 更多上下文：[流式传输 + 分块](https://docs.openclaw.ai/concepts/streaming)。
...
出站 Telegram API 调用在遇到临时网络/429 错误时会以指数退避和抖动进行重试。通过 `channels.telegram.retry` 配置。参见[重试策略](https://docs.openclaw.ai/concepts/retry)。
...
-   反应移除语义：参见 [/tools/reactions](https://docs.openclaw.ai/tools/reactions)。
...
**反应工作原理：** Telegram 反应作为**单独的 `message_reaction` 事件**到达，而不是消息负载中的属性。当用户添加反应时，OpenClaw：
...
智能体将反应视为对话历史中的**系统通知**，而不是消息元数据。 **配置：**
...
**论坛群组：** 论坛群组中的反应包含 `message_thread_id`，使用类似 `agent:main:telegram:group:{chatId}:topic:{threadId}` 的会话键。这确保同一话题中的反应和消息保持在一起。 **示例配置：**
...
[Slack](https://docs.openclaw.ai/zh-CN/channels/slack)[Tlon](https://docs.openclaw.ai/zh-CN/channels/tlon)
