<!-- Source: https://docs.openclaw.ai/zh-CN/channels/whatsapp -->
<!-- Title: WhatsApp - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
...
-   [WhatsApp（网页渠道）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [快速设置（新手）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [目标](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [配置写入](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [架构（谁拥有什么）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [获取手机号码（两种模式）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [专用号码（推荐）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [个人号码（备选方案）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [号码获取提示](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [为什么不用 Twilio？](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [登录 + 凭证](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [入站流程（私信 + 群组）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [个人号码模式（备选方案）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [已读回执](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [WhatsApp 常见问题：发送消息 + 配对](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [消息规范化（模型看到的内容）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [群组](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [回复投递（线程）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [确认表情（收到时自动回应）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [智能体工具（表情回应）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [限制](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [出站发送（文本 + 媒体）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [语音消息（PTT 音频）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [媒体限制 + 优化](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [心跳](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [重连行为](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [配置快速映射](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [日志 + 故障排除](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
-   [故障排除（快速）](https://docs.openclaw.ai/zh-CN/channels/whatsapp)
...
默认情况下，WhatsApp 允许写入由 `/config set|unset` 触发的配置更新（需要 `commands.config: true`）。 禁用方式：
...
为 OpenClaw 使用**单独的手机号码**。最佳用户体验，清晰的路由，无自聊天怪异问题。理想设置：**备用/旧 Android 手机 + eSIM**。保持 Wi-Fi 和电源连接，通过二维码关联。 **WhatsApp Business：** 你可以在同一设备上使用不同号码的 WhatsApp Business。非常适合将个人 WhatsApp 分开——安装 WhatsApp Business 并在那里注册 OpenClaw 号码。 **示例配置（专用号码，单用户允许列表）：**
...
快速备选方案：在**你自己的号码**上运行 OpenClaw。给自己发消息（WhatsApp”给自己发消息”）进行测试，这样就不会打扰联系人。在设置和实验期间需要在主手机上阅读验证码。**必须启用自聊天模式。** 当向导询问你的个人 WhatsApp 号码时，输入你将用于发送消息的手机（所有者/发送者），而不是助手号码。 **示例配置（个人号码，自聊天）：**
...
**避免：** TextNow、Google Voice、大多数”免费短信”服务——WhatsApp 会积极封锁这些。 **提示：** 该号码只需要接收一条验证短信。之后，WhatsApp Web 会话通过 `creds.json` 持久化。
...
如果你在**个人 WhatsApp 号码**上运行 OpenClaw，请启用 `channels.whatsapp.selfChatMode`（见上面的示例）。 行为：
...
默认情况下，Gateway 网关在接受入站 WhatsApp 消息后将其标记为已读（蓝色勾号）。 全局禁用：
...
**当我关联 WhatsApp 时，OpenClaw 会给随机联系人发消息吗？** 不会。默认私信策略是**配对**，因此未知发送者只会收到配对码，他们的消息**不会被处理**。OpenClaw 只会回复它收到的聊天，或你明确触发的发送（智能体/CLI）。 **WhatsApp 上的配对是如何工作的？** 配对是未知发送者的私信门控：
...
**多个人可以在一个 WhatsApp 号码上使用不同的 OpenClaw 实例吗？** 可以，通过 `bindings` 将每个发送者路由到不同的智能体（peer `kind: "dm"`，发送者 E.164 如 `+15551234567`）。回复仍然来自**同一个 WhatsApp 账户**，直接聊天会折叠到每个智能体的主会话，因此**每人使用一个智能体**。私信访问控制（`dmPolicy`/`allowFrom`）是每个 WhatsApp 账户全局的。参见[多智能体路由](https://docs.openclaw.ai/concepts/multi-agent)。 **为什么向导会询问我的手机号码？** 向导使用它来设置你的**允许列表/所有者**，以便允许你自己的私信。它不会用于自动发送。如果你在个人 WhatsApp 号码上运行，请使用相同的号码并启用 `channels.whatsapp.selfChatMode`。
...
-   **历史注入**（仅待处理）：
    -   最近_未处理_的消息（默认 50 条）插入在： `[Chat messages since your last reply - for context]`（已在会话中的消息不会重新注入）
...
WhatsApp 可以在收到传入消息时立即自动发送表情回应，在机器人生成回复之前。这为用户提供即时反馈，表明他们的消息已收到。 **配置：**
...
-   表情移除语义：参见 [/tools/reactions](https://docs.openclaw.ai/tools/reactions)。
...
-   **智能体心跳**可以按智能体配置（`agents.list[].heartbeat`）或通过 `agents.defaults.heartbeat` 全局配置（当没有设置按智能体条目时的降级）。
...
-   故障排除指南：[Gateway 网关故障排除](https://docs.openclaw.ai/gateway/troubleshooting)。
...
-   **不推荐** Bun。WhatsApp（Baileys）和 Telegram 在 Bun 上不可靠。 请使用 **Node** 运行 Gateway 网关。（参见入门指南运行时说明。）

[Twitch](https://docs.openclaw.ai/zh-CN/channels/twitch)[Zalo](https://docs.openclaw.ai/zh-CN/channels/zalo)
