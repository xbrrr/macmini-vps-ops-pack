<!-- Source: https://docs.openclaw.ai/zh-CN/channels/slack -->
<!-- Title: Slack - OpenClaw -->

[跳转到主要内容](https://docs.openclaw.ai/zh-CN/channels/slack)
...
-   [Slack](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [Socket 模式（默认）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [快速设置（新手）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [设置](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [OpenClaw 配置（最小）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [用户令牌（可选）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [令牌使用](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [历史上下文](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [HTTP 模式（Events API）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [设置](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [OpenClaw 配置（最小）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [Manifest（可选）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [权限范围（当前 vs 可选）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [Bot 令牌权限范围（必需）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [用户令牌权限范围（可选，默认只读）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [目前不需要（但未来可能需要）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [配置](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [限制](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [回复线程](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [按聊天类型的线程](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [手动线程标签](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [会话 + 路由](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [私信安全（配对）](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [群组策略](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [投递目标](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [工具操作](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [安全说明](https://docs.openclaw.ai/zh-CN/channels/slack)
-   [说明](https://docs.openclaw.ai/zh-CN/channels/slack)
...
使用下面的 manifest 以保持权限范围和事件同步。 多账户支持：使用 `channels.slack.accounts` 配置每个账户的令牌和可选的 `name`。参见 [`gateway/configuration`](https://docs.openclaw.ai/gateway/configuration) 了解共享模式。
...
OpenClaw 可以使用 Slack 用户令牌（`xoxp-...`）进行读取操作（历史记录、置顶、表情回应、表情符号、成员信息）。默认情况下保持只读：当存在用户令牌时，读取优先使用用户令牌，而写入仍然使用 bot 令牌，除非你明确选择加入。即使设置了 `userTokenReadOnly: false`，当 bot 令牌可用时，写入仍然优先使用 bot 令牌。 用户令牌在配置文件中配置（不支持环境变量）。对于多账户，设置 `channels.slack.accounts.<id>.userToken`。 包含 bot + app + 用户令牌的示例：
...
Slack 的 Conversations API 是按类型区分的：你只需要你实际接触的会话类型（channels、groups、im、mpim）的权限范围。概述参见 [https://docs.slack.dev/apis/web-api/using-the-conversations-api/。](https://docs.slack.dev/apis/web-api/using-the-conversations-api/%E3%80%82)
...
-   `chat:write`（通过 `chat.postMessage` 发送/更新/删除消息） [https://docs.slack.dev/reference/methods/chat.postMessage](https://docs.slack.dev/reference/methods/chat.postMessage)
-   `im:write`（通过 `conversations.open` 打开私信用于用户私信） [https://docs.slack.dev/reference/methods/conversations.open](https://docs.slack.dev/reference/methods/conversations.open)
-   `channels:history`、`groups:history`、`im:history`、`mpim:history` [https://docs.slack.dev/reference/methods/conversations.history](https://docs.slack.dev/reference/methods/conversations.history)
-   `channels:read`、`groups:read`、`im:read`、`mpim:read` [https://docs.slack.dev/reference/methods/conversations.info](https://docs.slack.dev/reference/methods/conversations.info)
-   `users:read`（用户查询） [https://docs.slack.dev/reference/methods/users.info](https://docs.slack.dev/reference/methods/users.info)
-   `reactions:read`、`reactions:write`（`reactions.get` / `reactions.add`） [https://docs.slack.dev/reference/methods/reactions.get](https://docs.slack.dev/reference/methods/reactions.get) [https://docs.slack.dev/reference/methods/reactions.add](https://docs.slack.dev/reference/methods/reactions.add)
-   `pins:read`、`pins:write`（`pins.list` / `pins.add` / `pins.remove`） [https://docs.slack.dev/reference/scopes/pins.read](https://docs.slack.dev/reference/scopes/pins.read) [https://docs.slack.dev/reference/scopes/pins.write](https://docs.slack.dev/reference/scopes/pins.write)
-   `emoji:read`（`emoji.list`） [https://docs.slack.dev/reference/scopes/emoji.read](https://docs.slack.dev/reference/scopes/emoji.read)
-   `files:write`（通过 `files.uploadV2` 上传） [https://docs.slack.dev/messaging/working-with-files/#upload](https://docs.slack.dev/messaging/working-with-files)
...
-   `chat:write.public`（仅当我们想发布到机器人未加入的频道时） [https://docs.slack.dev/reference/scopes/chat.write.public](https://docs.slack.dev/reference/scopes/chat.write.public)
-   `users:read.email`（仅当我们需要从 `users.info` 获取邮箱字段时） [https://docs.slack.dev/changelog/2017-04-narrowing-email-access](https://docs.slack.dev/changelog/2017-04-narrowing-email-access)
...
当未设置聊天类型覆盖时，旧版 `channels.slack.dm.replyToMode` 仍可作为 `direct` 的回退。 示例： 仅对私信使用线程：
...
-   完整命令列表 + 配置：[斜杠命令](https://docs.openclaw.ai/tools/slash-commands)
...
-   对于 Slack 工具，表情回应移除语义见 [/tools/reactions](https://docs.openclaw.ai/tools/reactions)。
-   附件在允许且在大小限制内时会下载到媒体存储。

[Signal](https://docs.openclaw.ai/zh-CN/channels/signal)[Telegram](https://docs.openclaw.ai/zh-CN/channels/telegram)
